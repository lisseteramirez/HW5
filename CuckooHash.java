...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        int count = 0; //will detect cycles when we place the elements
        K currentKey = key;
        V currentValue = value;
        //Loops to try to place the key-value pair in one of the two positions
        while (count < CAPACITY) {
            int pos1 = hash1(currentKey); //calculates the first hash position
            int pos2 = hash2(currentKey); //calculates the second hash position
            //will check if pos1 is empty and if so the key-value is placed there
            if (table[pos1] == null) {
                table[pos1] = new Bucket<>(currentKey, currentValue);
                return;
                //if pos2 is empty, the key-value will be placed there
            } else if (table[pos2] == null) {
                table[pos2] = new Bucket<>(currentKey, currentValue);
                return;
            }
            //if the same key-value is already there (in pos1 and pos2), then insertion is skipped
            if ((table[pos1].getBucKey().equals(currentKey) && table[pos1].getValue().equals(currentValue)) ||
                    (table[pos2].getBucKey().equals(currentKey) && table[pos2].getValue().equals(currentValue))) {
                return;
            }
            //if both positions are occupied, we kick out the element in pos1
            Bucket<K, V> displaced = table[pos1];
            table[pos1] = new Bucket<>(currentKey, currentValue); //the current key value will be placed in pos1
            currentKey = displaced.getBucKey();//displaced key is the new key we place
            currentValue = displaced.getValue();//displaced value is the new value we place
            count++;
        }
        //count will reach the capacity if cycle is detected
        rehash();
        put(currentKey, currentValue);// we try to insert the last displaced item again
    }
